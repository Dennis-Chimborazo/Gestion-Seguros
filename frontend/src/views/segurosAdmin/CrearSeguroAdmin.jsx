...
export function CrearSeguroAdmin({ mostrarSeccion }) {
    ...
    if (Object.values(formulario).every(valor => valor !== '')) {
      if (listbeneficios.length == 0) {
        ...
        try {
          ...
          await guardarBeneficios(res.data.id_tip_seg);
          swal.fire({
  ...
  const cancelar = () => {
    const algunCampoLleno = Object.values(formulario).some(valor => valor.trim() !== '') || listbeneficios.length > 0;
    if (algunCampoLleno) {
      swal.fire({
  ...
  // *** El return JSX estaba fuera de esta función ***
  return (
