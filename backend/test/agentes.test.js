...
// Configurar mocks ANTES de importar cualquier módulo
const mockDatabase = {
  query: jest.fn()
};
...
describe('Agente Router - Pruebas de Integración', () => {
  ...
  describe('GET /agente/listarPendientes', () => {
    it('debería listar agentes pendientes exitosamente', async () => {
      const mockAgentesPendientes = {
        rows: [
          {
            id_agente: 2,
            ced_agente: '0987654321',
            nom_agente: 'María',
            ape_agente: 'González',
            id_estado: '3'
          }
      ...
      mockDatabase.query.mockResolvedValue(mockAgentesPendientes);

      const response = await request(app)
        .get('/agente/listarPendientes')
        .expect(200);

      expect(response.body).toEqual(mockAgentesPendientes);
      expect(mockDatabase.query).toHaveBeenCalledWith(
        'SELECT * FROM agente WHERE id_estado = $1',
        [3]
      );
  ...
  describe('POST /agente/buscar-ruta-token', () => {
    it('debería encontrar ruta de token válida', async () => {
      const validToken = jwt.sign({ id_pers: 1 }, 'emailAgente', { expiresIn: '1h' });
      
      mockDatabase.query.mockResolvedValue({
        rowCount: 1,
        rows: [{
          token_val: validToken,
          url_emal: 'test-url'
        }]
      });

      const response = await request(app)
        .post('/agente/buscar-ruta-token')
        .send({ id: 1 })
        .expect(200);
    ...
    it('debería rechazar token expirado', async () => {
      const expiredToken = jwt.sign({ id_pers: 1 }, 'emailAgente', { expiresIn: '-1h' });
      
      mockDatabase.query.mockResolvedValue({
        rowCount: 1,
        rows: [{
          token_val: expiredToken,
          url_emal: 'test-url'
        }]
      });
    ...
    it('debería manejar agente sin URL asociada', async () => {
      mockDatabase.query.mockResolvedValue({ rowCount: 0 });
  ...
  describe('Pruebas de flujo completo', () => {
    it('debería completar el flujo de registro y activación de agente', async () => {
      const nuevoAgente = {
        ced_agente: '1234567890',
        nom_agente: 'Test',
        ape_agente: 'User',
        email_agente: 'test@email.com',
        dire_agente: 'Test Address',
        tel_agente: '0991234567'
      };

      // 1. Crear agente
      mockDatabase.query.mockResolvedValueOnce({ rows: [] }); // No existe
      mockDatabase.query.mockResolvedValueOnce({ rows: [{ id_agente: 1 }] }); // Creado
      ...
      expect(createResponse.body.id_agente).toBe(1);
      ...
      mockDatabase.query.mockResolvedValueOnce({
        rowCount: 1,
        rows: [{
          token_val: tokenResponse.body.token,
          id_val: 1
        }]
      });
      ...
      const validationResponse = await request(app)
        .post('/agente/validar-token-email')
        .send({ url: 'activation-url' })
        .expect(200);

      expect(validationResponse.body.success).toBe(true);

      // 4. Activar cuenta
      mockDatabase.query.mockResolvedValueOnce({ rowCount: 1 }); // UPDATE
      mockDatabase.query.mockResolvedValueOnce({ rowCount: 1 }); // DELETE

      const activationResponse = await request(app)
        .put('/agente/activar-cuenta')
        .send({
          id: 1,
          idvalid: validationResponse.body.idvalid
        })
        .expect(200);

      expect(activationResponse.body.message).toBe('Cuenta activada con éxito.');
    });
  });
});